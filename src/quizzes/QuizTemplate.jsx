...
const QuizTemplate = ({ quiz, title }) => {
  ...
  return (
    <>
      {showStarterPrompt ? (
        <StartQuizBtn showQuiz={displayQuiz} title={quizTitle} />
      ) : (
        <>
          {showQuizPage ? (
            <QuizQuestions
        ...
        <div className="lg:mt-0 lg:flex-shrink-0">
          <div className="mt-12 inline-flex rounded-md shadow">
            <button
              style={btnStyles}
              onClick={showQuiz}
              className="py-4 px-6  bg-purple-700  text-white w-full  rounded-lg "
            >
              Start quiz
            </button>
          </div>
...
const QuizQuestions = ({ quizQuestion, nextQuestion }) => {
  return (
    <div className="mb-16">
      <h2 className="text-4xl mt-6 mb-2 text-center">
        {quizQuestion.question}
      </h2>

      <div className="container text-center mx-auto p-4">
        <div
          className="flex flex-col justify-center rounded-lg  my-2"
          role="group"
        >
          {quizQuestion.answers.map((answer, index) => (
            <div key={index}>
              <button
                className="bg-purple-700 my-2 text-2xl text-white hover:bg-green-700 rounded px-4 outline-none focus:shadow-outline"
...
const ResultsPage = ({
  ...
  return (
    <div
      style={{ margin: "auto" }}
      className="flex  justify-center content-center item-center max-w-md bg-white border border-purple-700  rounded-lg overflow-hidden"
    >
      <div className="w-2/3 my-4 p-4">
        <h2 className="text-black-900 mb-12 text-center font-bold text-2xl">
          {" "}
          Results{" "}
        </h2>
        <p className="mt-2 text-center text-black-600 text-xl">
          {totalCorrect} out of {quiz.length} correct{" "}
        </p>

        <div className="flex-col text-center item-center justify-between my-4">
          <p>
            {" "}
            {userPoints} points out of {totalPoints} points{" "}
          </p>
          <button
